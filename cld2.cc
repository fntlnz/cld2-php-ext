...
PHP_METHOD(cld2_detector, detect)
{
    ...
    if (zend_parse_method_parameters(ZEND_NUM_ARGS(), getThis(), "Os", &detector, cld2_detector_ce, &text, &text_len) == FAILURE) {
        RETURN_NULL();
    }
    zval rv1, rv2, rv3, rv4;

    is_plain = zend_read_property(cld2_detector_ce, Z_OBJ_P(detector), "isPlainText", sizeof("isPlainText") - 1, 1, &rv1);
    tld_hint = zend_read_property(cld2_detector_ce, Z_OBJ_P(detector), "tldHint", sizeof("tldHint") - 1, 1, &rv2);
    language_hint = zend_read_property(cld2_detector_ce, Z_OBJ_P(detector), "languageHint", sizeof("languageHint") - 1, 1, &rv3);
    encoding_hint = zend_read_property(cld2_detector_ce, Z_OBJ_P(detector), "encodingHint", sizeof("encodingHint") - 1, 1, &rv4);
...
PHP_METHOD(cld2_detector, isPlainText)
{
    ...
    if (zend_parse_method_parameters(ZEND_NUM_ARGS(), getThis(), "O", &detector, cld2_detector_ce) == FAILURE) {
        RETURN_NULL();
    }
    zval rv;
    is_plain = zend_read_property(cld2_detector_ce, Z_OBJ_P(detector), "isPlainText", sizeof("isPlainText") -1, 1, &rv);
...
PHP_METHOD(cld2_detector, setPlainText)
{
    ...
    if (zend_parse_method_parameters(ZEND_NUM_ARGS(), getThis(), "Ob", &detector, cld2_detector_ce, &is_plain) == FAILURE) {
        RETURN_NULL();
    }

    zend_update_property_bool(cld2_detector_ce, Z_OBJ_P(detector), "isPlainText", sizeof("isPlainText") - 1, is_plain);
} /* }}} */
...
PHP_METHOD(cld2_detector, getTldHint)
{
    ...
    zval rv;
    tld_hint = zend_read_property(cld2_detector_ce, Z_OBJ_P(detector), "tldHint", sizeof("tldHint") - 1, 1, &rv);
...
PHP_METHOD(cld2_detector, setTldHint)
{
    ...
    zend_update_property_string(cld2_detector_ce, Z_OBJ_P(getThis()), "tldHint", sizeof("tldHint") - 1, hint);
} /* }}} */
...
PHP_METHOD(cld2_detector, getLanguageHint)
{
    ...
    languageHint = zend_read_property(cld2_detector_ce, Z_OBJ_P(detector), "languageHint", sizeof("languageHint") - 1, 1, &rv);
...
PHP_METHOD(cld2_detector, setLanguageHint)
{
    ...
    if (zend_parse_method_parameters(ZEND_NUM_ARGS(), getThis(), "Ol", &detector, cld2_detector_ce, &hint) == FAILURE) {
        RETURN_NULL();
    }

    zend_update_property_long(cld2_detector_ce, Z_OBJ_P(detector), "languageHint", sizeof("languageHint") - 1, hint);
} /* }}} */
...
PHP_METHOD(cld2_detector, setEncodingHint)
{
    ...
    zend_update_property_long(cld2_detector_ce, Z_OBJ_P(detector), "encodingHint", sizeof("encodingHint") - 1, hint);
} /* }}} */
...
PHP_METHOD(cld2_detector, getEncodingHint)
{
    ...
    encodingHint = zend_read_property(cld2_detector_ce, Z_OBJ_P(detector), "encodingHint", sizeof("encodingHint") -1, 1, &rv);
...
PHP_METHOD(cld2_language, languageName)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "l", &language) == FAILURE) {
        RETURN_NULL();
    }
...
PHP_METHOD(cld2_encoding, encodingName)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "l", &encoding) == FAILURE) {
        RETURN_NULL();
    }
...
PHP_MINIT_FUNCTION(cld2)
{
    ...
    cld2_detector_ce = zend_register_internal_class(&ce);
    zend_declare_property_bool(cld2_detector_ce, "isPlainText", sizeof("isPlainText") - 1 , 0, ZEND_ACC_PUBLIC);
    zend_declare_property_string(cld2_detector_ce, "tldHint", sizeof("tldHint") - 1, "", ZEND_ACC_PUBLIC);
    zend_declare_property_long(cld2_detector_ce, "languageHint", sizeof("languageHint") - 1, CLD2::UNKNOWN_LANGUAGE, ZEND_ACC_PUBLIC);
    zend_declare_property_long(cld2_detector_ce, "encodingHint", sizeof("encodingHint") - 1, CLD2::UNKNOWN_ENCODING, ZEND_ACC_PUBLIC);

    // Language
    zend_class_entry ce_Language;
    INIT_CLASS_ENTRY(ce_Language, "CLD2Language", cld2_language_methods);
    cld2_language_ce = zend_register_internal_class(&ce_Language);
    ...
    cld2_encoding_ce = zend_register_internal_class(&ce_Encoding);

    for (int i = 0; i < CLD2::NUM_ENCODINGS; i++) {
        CLD2::Encoding enc = (CLD2::Encoding) (i);
        zend_declare_class_constant_long(cld2_encoding_ce, encodingStrings[i], strlen(encodingStrings[i]), i);
    }
